...
class RelayServerTests(unittest.TestCase):
    ...
    def test_public_home_legal_and_support_pages(self):
        with TestClient(self.app) as client:
            ...
            self.assertIn("keep working on machines you control", home.text)
            self.assertIn("No more status babysitting", home.text)
            self.assertIn("github.com/jboone1989/livingruntime-remote", home.text)
            ...
            self.assertIn("Durable continuation walkthrough", demo.text)
            self.assertIn("docs/DEMO.md", demo.text)
