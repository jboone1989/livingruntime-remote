...
SQLite is sufficient for one relay process and the initial reviewed beta. Before horizontal scaling, replace the task queue with a shared broker/database while preserving the same user/device ownership checks. Do not put sticky SSH credentials in the relay to avoid building a second secret-management plane.
